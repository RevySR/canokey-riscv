...
static int write_block(const struct lfs_config *c, lfs_block_t b, lfs_off_t o, const void *p, lfs_size_t n) {
    ...
    if (!bounds(b, o, n)) return LFS_ERR_INVAL;
    const uint8_t *in = p;
    for (lfs_size_t i = 0; i < n; ++i) disk[b][o+i] &= in[i];
    return 0;
}
...
static const struct lfs_config config = {
    ...
    .read_size = 1, .prog_size = 8, .block_size = BLOCK_SIZE, .block_count = BLOCK_COUNT,
    .block_cycles = 1000, .cache_size = LFS_CACHE_SIZE, .lookahead_size = sizeof(lookahead),
